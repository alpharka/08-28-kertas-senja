...
type GuestEntry = { name: string; status: string; message: string; createdAt: string };

function readGuestbook(): GuestEntry[] {
  try {
    const parsed = JSON.parse(localStorage.getItem("kertas-senja-rsvp") || "[]");
    return Array.isArray(parsed) ? parsed.filter((entry) => entry && typeof entry.name === "string" && typeof entry.message === "string") : [];
  } catch {
    return [];
  }
}
...
export default function Home() {
  ...
  const [entries, setEntries] = useState<GuestEntry[]>(readGuestbook);
  ...
  const submitRsvp = (e: React.FormEvent) => { e.preventDefault(); const name = rsvp.name.replace(/\s+/g, " ").trim(); const message = rsvp.message.replace(/\s+/g, " ").trim(); if (name.length < 2 || message.length < 4) { setFormError("Mohon isi nama dan ucapan dengan lengkap agar dapat ditampilkan di buku tamu."); setSent(false); return; } const next: GuestEntry[] = [...entries, { name: name.slice(0, 80), status: rsvp.status, message: message.slice(0, 400), createdAt: new Date().toISOString() }]; setEntries(next); localStorage.setItem("kertas-senja-rsvp", JSON.stringify(next)); setFormError(""); setSent(true); setRsvp({ name: "", status: "Hadir", message: "" }); };
  ...
  return <div ref={revealRef} className="site-shell">
    ...
    <main id="top">
      ...
      <section id="rsvp" className="rsvp section-light"><div className="section-rail"><span>04</span><span>RSVP</span></div><div className="rsvp__content"><div data-reveal><span className="paper-label">Catatan kehadiran · 04</span><p className="eyebrow accent">Kolom ucapan</p><h2>Akankah<br /><em>kau hadir?</em></h2><p className="rsvp__intro">Bantu kami menyiapkan hari ini dengan mengabarkan kehadiranmu. Pesanmu akan tersimpan di perangkat ini.</p></div><form onSubmit={submitRsvp} data-reveal><label>Nama lengkap<input value={rsvp.name} onChange={(e) => { setRsvp({ ...rsvp, name: e.target.value }); setFormError(""); }} placeholder="Tulis namamu" minLength={2} maxLength={80} required /></label><fieldset><legend>Konfirmasi kehadiran</legend><label className="choice"><input type="radio" name="status" checked={rsvp.status === "Hadir"} onChange={() => setRsvp({ ...rsvp, status: "Hadir" })} /> Saya akan hadir</label><label className="choice"><input type="radio" name="status" checked={rsvp.status === "Belum pasti"} onChange={() => setRsvp({ ...rsvp, status: "Belum pasti" })} /> Belum bisa memastikan</label><label className="choice"><input type="radio" name="status" checked={rsvp.status === "Tidak hadir"} onChange={() => setRsvp({ ...rsvp, status: "Tidak hadir" })} /> Tidak dapat hadir</label></fieldset><label>Pesan ucapan<textarea value={rsvp.message} onChange={(e) => { setRsvp({ ...rsvp, message: e.target.value }); setFormError(""); }} placeholder="Tulis doa baikmu untuk kami" minLength={4} maxLength={400} required rows={4} /></label><button className="button button--dark" type="submit">Kirim ucapan <ArrowDownRight size={16} /></button>{formError && <p className="form-error" role="alert">{formError}</p>}{sent && <p className="success"><Check size={15} /> Terima kasih, konfirmasimu sudah tersimpan di perangkat ini.</p>}</form><div className="guestbook" data-reveal><p className="eyebrow accent">Buku tamu</p>{entries.length === 0 ? <p className="empty-state">Pesan ucapanmu akan muncul di sini setelah dikirim.</p> : entries.map((entry, i) => <div className="guest-entry" key={i}><div><strong>{entry.name}</strong><span>{entry.status}</span></div><p>{entry.message}</p></div>)}</div></div></section>
